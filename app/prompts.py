...
输出要求：
1) 篇幅严格控制在 350 字以内，这是硬性上限。
2) 先用1句话点明本次讲解主题。
3) 再用2个小节总结核心知识点，语言口语化、适合语音播报。
4) 只讲结论与方法，不展开举例、不复述定义原文、不逐字照搬PDF。
5) 不要透露、复述或讲解教辅 PDF 中的任何例题、习题、练习原题；只做知识点归纳。
...
def teach_invite_prompt(grade: str, subject: str, lesson_text: str) -> str:
    return f"""
你是一位热心的初中{subject}同学，想请对方给你讲讲这道题。请基于讲解内容，生成一句邀请对方向你讲题的话。
        ...
        json_schema = """
{
  ...
  "closing_message": "本次互讲收尾语（1句，积极简短）"
    ...
    return f"""
你是一位初中{subject}同学，正在听对方给你讲题。请结合对话历史评估对方本轮讲解，并用同伴的口吻给鼓励式反馈。
